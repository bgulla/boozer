...
#        logger.info("event-bus: registered tap " + str(self.get_tap_id()) + " successfully")
		logger.info("Tap[%i] Pour Event: %s pints." %( self.get_tap_id(), str(round(self.totalPour,3))))
	...
	def listen_for_pour(self):
		...
		if self.thisPour > 0.0:
			...
			if (self.thisPour > self.minimum_pour_vol and currentTime - self.lastClick > 10000):  # 10 seconds of inactivity causes a tweet
				logger.info("--- REGISTERING-FULL-POUR   %s vs %s " % (str(self.thisPour), str(self.minimum_pour_vol)) ) 
				self.register_new_pour(currentTime)
				self.last_event_type = POUR_FULL # set last event status for event bus in boozer
			else:
				logger.debug("--- Pour -> %s vs Threshold -> %s " % (str(self.thisPour), str(self.minimum_pour_vol)) ) 
